    ...
    >
      ...
      <div className="glass-panel rounded-xl overflow-hidden shadow-2xl relative">
        <div className="px-6 py-3 border-b border-border/50 flex items-center justify-between bg-white/[0.02]">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-secondary/30">
            rank.zbr
          </span>
        </div>
            ...
            <code className="text-foreground/80">
              {code.split("\n").map((line, i) => (
                <div key={i} className="table-row">
                  <span className="table-cell pr-6 text-secondary/20 select-none text-right w-8">
                    {i + 1}
                  </span>
                  <span className="table-cell whitespace-pre">
                    {line
                      ...
                      .map((part, j) => {
                        if (part.startsWith("//"))
                          return (
                            <span key={j} className="text-secondary/30">
                              {part}
                            </span>
                          );
                        if (part.startsWith("#"))
                        ...
                        if (part.startsWith("Z"))
                          return (
                            <span key={j} className="text-[#9dc3f7] font-bold">
                              {part}
                            </span>
                          );
                        if (part === "{" || part === "}")
