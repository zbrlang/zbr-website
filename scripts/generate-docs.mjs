...
async function generateDocs() {
  ...
  functions.forEach((fn) => {
    const categoryName =
      functionToCategory[fn.name] || fn.category || "General";
    const categoryDirName = categoryName
      .toLowerCase()
      .replace(/ & /g, "-")
      .replace(/ /g, "-");
...
`;
  ...
  fs.writeFileSync(
    path.join(functionsDir, "_meta.tsx"),
    `const meta = ${JSON.stringify(categories, null, 2)};\n\nexport default meta;`,
  );

  // Create meta files for each category
  Object.entries(categoryMetas).forEach(([dirName, meta]) => {
    fs.writeFileSync(
      path.join(functionsDir, dirName, "_meta.tsx"),
...
| :--- | :--- |
${Object.entries(trigger.context)
  .map(([key, val]) => `| ${key} | ${val} |`)
  .join("\n")}
`
    : ""
...
`;
  fs.writeFileSync(
    path.join(interactionDir, "onInteraction.mdx"),
    interactionContent,
  );

  // Create triggers meta
  fs.writeFileSync(
    path.join(eventsDir, "_meta.tsx"),
    `const meta = ${JSON.stringify(eventsMeta, null, 2)};\n\nexport default meta;`,
  );
  fs.writeFileSync(
    path.join(interactionDir, "_meta.tsx"),
    `const meta = { "onInteraction": "onInteraction" };\n\nexport default meta;`,
  );
  fs.writeFileSync(
    path.join(triggersDir, "_meta.tsx"),
    `const meta = {
  "events": "Events",
