...
## v1.8.5 - Distribution Simplification

This release reverts the experimental distribution changes introduced in v1.8.4, returning ZBR to its simpler NPM/Bun and manual download distribution model.

### Distribution

* **NPM/Bun**: Retained NPM and Bun as the primary installation methods.
* **Manual Downloads**: Retained direct binary downloads for users who prefer manual installation.
* **Removed Experimental Support**: Removed Homebrew, AUR, Winget, Scoop, and shell installer distribution.

### Infrastructure & Documentation

* **CI/CD**: Simplified the release pipeline to focus on core NPM publishing and binary releases.
* **Documentation**: Updated installation documentation to reflect the reverted distribution methods.

## v1.8.4 - Multi-Channel Distribution & Automated CI/CD

This release introduces comprehensive multi-channel distribution support for the ZBR CLI, along with fully automated CI/CD pipeline integration.

### Distribution Channels

- **Homebrew (macOS/Linux)**: Added support for installation via Homebrew tap (`brew install zbrlang/tap/zbr`).
- **Scoop (Windows)**: Added support for installation via Scoop bucket (`scoop bucket add zbrlang ...`).
- **AUR (Arch Linux)**: Added support for installation via the Arch User Repository (`yay -S zbr`).
- **Winget (Windows)**: Added support for installation via the Windows Package Manager (`winget install zbrlang.zbr`).
- **Shell Installer**: Introduced a native shell installer (`curl -sSfL https://install.zbrlang.tech | sh`) for Linux and macOS.

### Infrastructure & Automation

- **Automated CI/CD Pipeline**: Enhanced the `release.yml` GitHub Actions workflow to automatically build, calculate binary hashes, update manifests, and deploy to Homebrew, Scoop, AUR, and Winget repositories upon new releases.
- **Subdomain Redirection**: Updated website `middleware.ts` to support automated redirects for `homebrew.`, `scoop.`, `aur.`, `winget.`, `install.`, and `packages.` subdomains to their respective GitHub repositories or download assets.
- **Documentation**: Centralized all installation instructions into a new, comprehensive guide and standardized across `DOWNLOADS.md` and website documentation (`installation.mdx`).
- **NPM/Bun Optimization**: Refined NPM packaging and documented support for Bun installation.

## v1.8.3 - Discord API Feature Expansion

This release adds 7 new Discord API-based functions for Events, Polls, and Onboarding management.

### New Functions

- **`ZeventInvite{eventID}`**: Returns a shareable invite URL for the scheduled event.
- **`ZeventUpdateStatus{eventID;status}`**: Updates the status of a scheduled event.
- **`ZeventCover{eventID;imageURL?}`**: Gets or sets the cover image URL for a scheduled event.
- **`ZpollAnswerDetails{pollID;answerID}`**: Returns JSON with answer text, emoji, and vote count for a specific answer in a poll.
- **`ZpollExpiration{pollID;endTime?}`**: Gets or sets the poll's end timestamp.
- **`ZonboardingSetMode{mode}`**: Sets the onboarding mode for the guild.
- **`ZonboardingSetEnabled{enabled}`**: Enables or disables guild onboarding.

### API Expansion

- Added base function stubs for Event management (invite generation, status updates, and cover management).
- Added base function stubs for Poll interaction (answer details and expiration settings).
- Added base function stubs for Guild Onboarding configuration (mode and status toggles).
...
- **`Zai{apiKey;prompt;model?;maxTokens?;temperature?}`**: Sends a prompt to the AI and returns the response text. Supports Gemini API keys (AIza... or AQ...).
- **`ZaiCtx{apiKey;prompt;model?;maxTokens?;temperature?}`**: Same as `Zai` but automatically injects Discord context (author, server, channel, message) as a system message.
- **`ZaiDecide{apiKey;content;question;model?}`**: Asks the AI a yes/no question about the provided content. Returns strictly `true` or `false`.
- **`ZaiDecideCtx{apiKey;question;model?}`**: Same as `ZaiDecide` but uses the current Discord context instead of provided content.
- **`ZaiExtract{apiKey;content;instruction;model?}`**: Extracts specific information from text based on the instruction (e.g., emails, phone numbers, usernames). Returns `N/A` if not found.
- **`ZaiExtractCtx{apiKey;instruction;model?}`**: Same as `ZaiExtract` but extracts from the current Discord context.
- **`ZaiClassify{apiKey;content;categories;model?}`**: Classifies content into one category from a comma-separated list. Returns the best matching category.
- **`ZaiClassifyCtx{apiKey;categories;model?}`**: Same as `ZaiClassify` but classifies the current Discord context.
- **`ZimageAnalyze{apiKey;imageUrl;prompt;model?}`**: Analyzes an image from a URL using AI vision and returns a text description based on the prompt.

### AI Features

- Default model: `gemini-2.5-flash-lite` (free tier compatible)
- Supports both `AIza...` and `AQ...` API key formats
- Uses `v1beta` API endpoint for maximum model compatibility
- Temperature and token limits configurable per function
